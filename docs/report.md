...
## Часть 1. Проектирование архитектуры «To Be» (Целевая)
...
### 4. Показатели качества (Non-functional requirements)
...
*(Здесь должно быть место для диаграммы компонентов, показывающей, как UI, Kivy App, Core, и DBManager взаимодействуют в рамках одного процесса)*
...
## Часть 2. Анализ архитектуры «As Is» (Реальность в коде)

В данной части анализируется текущая структура проекта, которая представляет собой **Слойчатый Монолит (Layered Monolith)**.

**(Здесь можно вставить Диаграмму классов, сгенерированную из вашего кода, сфокусированную на `ReaderScreen` и `DBManager`)**
...
### Часть 3. Пути улучшения архитектуры (Рефакторинг)
